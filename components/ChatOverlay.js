...
export default function ChatOverlay({ onClose }) {
  ...
  async function updateCardField(msgId, field, value) {
    ...
    // عند اختيار الموقع: نملأ السعر تلقائيًا من السعر المحفوظ على الموقع نفسه في الإعدادات
    if (field === "location_name" && value) {
      try {
        const price = await lookupLocationPrice(value);
        if (price !== null) {
          setMessages((prev) =>
    ...
    // عند إدخال رقم فاتورة: نتحقق من التكرار ونحذّر داخل البطاقة
    if (field === "invoice_number" && value && String(value).trim()) {
      try {
        const dup = await checkDuplicateInvoice(value);
        setMessages((prev) =>
          prev.map((x) =>
            x.id === msgId
              ? {
                  ...
                  payload: {
                    ...x.payload,
                    dupWarning: dup ? `تنبيه: رقم الفاتورة "${value}" مستخدم من قبل في ${dup.where} للطرف "${dup.party}" بتاريخ ${dup.date}` : "",
                  },
                }
              : x
          )
  ...
  async function approveCard(msgId) {
    ...
    // منع الاعتماد بلا إجمالي: إما (كمية × سعر) أو مبلغ مُدخل يدويًا
    const f = m.payload.fields;
    const computed = num(f.quantity) !== null && num(f.unit_price) !== null ? num(f.quantity) * num(f.unit_price) : null;
    const manual = num(f.amount);
    if (computed === null && manual === null) {
      alert("لا يمكن اعتماد العملية بلا إجمالي.\nأدخل (الكمية والسعر) معًا، أو أدخل المبلغ يدويًا.");
      return;
    }

    // تحذير أخير عند تكرار رقم الفاتورة قبل الحفظ
    if (f.invoice_number && String(f.invoice_number).trim()) {
      try {
        const dup = await checkDuplicateInvoice(f.invoice_number);
        if (dup) {
          const ok = confirm(
            `رقم الفاتورة "${f.invoice_number}" مسجّل من قبل في ${dup.where} للطرف "${dup.party}" بتاريخ ${dup.date}.\n\nهل تريد المتابعة والحفظ رغم ذلك؟`
          );
          if (!ok) return;
        }
      } catch (e) {}
...
function OperationCard({ msg, opTypes, lists, onUpdateField, onUpdateType, onBlurPersist, onApprove, onCancel }) {
  ...
  return (
    ...
    >
      ...
      {msg.payload.dupWarning && !isSaved && (
        <div style={{ background: "#3a2418", color: "#e8a87c", padding: 8, borderRadius: 8, fontSize: 11, marginBottom: 8 }}>🔁 {msg.payload.dupWarning}</div>
      )}

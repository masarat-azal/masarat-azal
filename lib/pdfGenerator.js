...
export async function generateShapedStatement({ shapeTitleAr, shapeTitleEn, partyName, partyLabel, periodText, columns, rows, totalsRows, itemTotals, landscape }) {
  ...
  rows.forEach((r, i) => {
    ...
    const cells = columns
      .map((c) => {
        ...
        const val = r[c.key] ?? "—";
        const small = c.key === "notes" ? `font-size:8px;color:${GREEN};opacity:0.75;` : `font-size:10px;color:${GREEN};font-weight:600;`;
  ...
  if (itemTotals && itemTotals.length) {
    itemTotalsHtml = `<table style="width:100%;border-collapse:collapse;margin-top:16px;">
      ...
      ${itemTotals.map((it) => `<tr><td style="padding:6px;text-align:center;font-size:10px;color:${GREEN};font-weight:600;">${it.item}</td><td style="padding:6px;text-align:center;font-size:10px;color:${GREEN};font-weight:600;">${it.qty}</td><td style="padding:6px;text-align:center;font-size:10px;color:${GREEN};font-weight:600;">${it.amount}</td></tr>`).join("")}
    </table>`;
  }
...
export async function generateQuickImage({ partyName, rows, totalsRows }) {
  ...
  const rowsHtml = rows
    .map(
      ...
      <td style="padding:8px;text-align:center;font-size:12px;color:${GREEN};font-weight:600;">${r.date}</td>
      <td style="padding:8px;text-align:center;font-size:12px;color:${GREEN};font-weight:600;">${r.item}</td>
      <td style="padding:8px;text-align:center;font-size:12px;color:${GREEN};font-weight:600;">${r.price}</td>
      <td style="padding:8px;text-align:center;font-size:12px;color:${GREEN};font-weight:600;">${r.total}</td>
      <td style="padding:8px;text-align:center;font-size:12px;color:${GREEN};font-weight:600;">${r.paid}</td>
      <td style="padding:8px;text-align:center;font-size:12px;color:${GREEN};font-weight:600;">${r.due}</td>
    </tr>`
    )
    .join("");

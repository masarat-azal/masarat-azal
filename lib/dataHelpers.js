...
/** يبحث عن سعر محفوظ لعميل/مورد عند موقع معيّن؛ يُستخدم لتعبئة السعر تلقائيًا عند اختيار الموقع. */
export async function lookupPrice(partyType, partyId, locationId) {
  let q = supabase.from("prices").select("unit_price").eq("party_type", partyType).eq("party_id", partyId);
  q = locationId ? q.eq("location_id", locationId) : q.is("location_id", null);
  const { data } = await q.limit(1);
  return data && data.length ? Number(data[0].unit_price) : null;
}

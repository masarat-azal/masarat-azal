...
function Card({ label, value, color }) {
  return (
    <div style={{ background: "#fff", borderRadius: 14, padding: 16, border: `1px solid ${COLORS.silver}` }}>
      <div style={{ fontSize: 12, color: COLORS.grey, marginBottom: 6, fontWeight: 600 }}>{label}</div>
...
export default function Dashboard() {
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        setTotals({
          totalSales,
          receivable: totalSales - paidByCustomers,
          totalPurchases,
          payable: totalPurchases - paidToSuppliers,
          totalExpenses,
  ...
  return (
    <div>
      <h1 style={{ fontSize: 20, fontWeight: 800, color: COLORS.green, marginBottom: 16 }}>لوحة التحكم</h1>

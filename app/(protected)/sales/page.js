...
import { buildGlobalLedger, updateSale, deleteSale, deleteCustomOperation, deletePayment } from "../../../lib/dataHelpers";
...
export default function SalesPage() {
  ...
  async function load() {
    setLoading(true);
    const data = await buildGlobalLedger("customer");
    setRows(data);
    setLoading(false);
  }
  ...
  async function saveEdit() {
    ...
    try {
      ...
      if (qty === null || price === null) throw new Error("الكمية والسعر مطلوبان");
  ...
  async function removeRow(r) {
    ...
    try {
      if (r.op_type_name === "بيع") await deleteSale(r._id);
      else if (r.op_type_name === "دفعة من عميل") await deletePayment(r._id);
  ...
  return (
    <div>
      <h1 style={{ fontSize: 20, fontWeight: 800, color: COLORS.gold, marginBottom: 16 }}>المبيعات — كشف المعاينة</h1>

      {editing && (
        <div style={{ background: COLORS.panel, border: `1.5px solid ${COLORS.gold}`, borderRadius: 12, padding: 16, marginBottom: 16 }}>
          <div style={{ fontWeight: 700, color: COLORS.gold, marginBottom: 12 }}>تعديل عملية بيع</div>
          ...
          <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 12, minWidth: 1000 }}>
            <thead>
              <tr style={{ background: COLORS.panel }}>
                {["#", "رقم العملية", "التاريخ", "العميل", "العملية", "الصنف", "الكمية", "الموقع", "السعر", "الإجمالي", "المدفوع", "المتبقي", "الفاتورة", "ملاحظات", ""].map((h, i) => (

...
export default function NewOperationPage() {
  ...
  async function handleImageSelected(e) {
    ...
    try {
      ...
      if (parsed.type === "غير_مفهوم") {
        setAiError("لم يستطع الذكاء الاصطناعي فهم محتوى الصورة بوضوح كافٍ. جرّب صورة أوضح أو الإدخال اليدوي.");
        return;
      }
      setDraft({ ...parsed, missing: parsed.missing || [] });
      setScreen("review");
  ...
  return (
    <div style={{ maxWidth: 480, margin: "0 auto" }}>
      ...
      {listError && (
        <div style={{ background: "#3a1f1f", color: COLORS.red, padding: 12, borderRadius: 10, marginBottom: 14, fontSize: 13 }}>⚠️ {listError}</div>
      )}
        ...
        <>
          <div style={{ background: COLORS.panel, borderRadius: 16, padding: 18, marginBottom: 14, border: `1px solid ${COLORS.border}` }}>
            ...
            <button onClick={runAI} disabled={aiLoading || !aiText.trim() || loadingLists} style={{ ...btnPrimary, width: "100%", marginTop: 10, opacity: aiLoading || loadingLists ? 0.6 : 1 }}>
              {aiLoading ? "⏳ جاري المعالجة…" : loadingLists ? "⏳ تحميل البيانات…" : "تحليل واعتماد"}
            </button>
            ...
            <button
              ...
              disabled={aiLoading}
              style={{ ...btnGhost, width: "100%", borderColor: COLORS.gold, color: COLORS.gold, opacity: aiLoading ? 0.6 : 1 }}
            >
              📷 رفع صورة فاتورة أو إيصال
            </button>
      ...
      {screen === "custom" && (
        <div style={{ background: COLORS.panel, borderRadius: 16, padding: 18, border: `1px solid ${COLORS.border}` }}>
          ...
          <div style={{ display: "flex", gap: 10 }}>
            <button onClick={saveCustom} disabled={saving} style={{ ...btnPrimary, flex: 1, opacity: saving ? 0.6 : 1 }}>
              {saving ? "⏳ جاري الحفظ…" : "✅ حفظ"}
            </button>
            <button onClick={() => setScreen("home")} disabled={saving} style={{ ...btnGhost, flex: 1 }}>
              ❌ إلغاء
            </button>
          </div>
        </div>
      ...
      {screen === "review" && draft && (
        <div style={{ background: COLORS.panel, borderRadius: 16, padding: 18, border: `1px solid ${COLORS.border}` }}>
          ...
          {draft.missing?.length > 0 && (
            <div style={{ background: "#3a331a", color: "#e0c88a", padding: 10, borderRadius: 10, fontSize: 13, marginBottom: 14 }}>
